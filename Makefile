...
ifneq ($(IS_CI),false)
	...
	$(GO) run ./build/docker-compose-generator/main.go $(ENABLED_DOCKER_SERVICES) | (docker-compose -f docker-compose.makefile.yml build && docker-compose -f docker-compose.makefile.yml -f /dev/stdin run --rm start_dependencies)
